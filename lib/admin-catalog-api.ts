...
export async function handleCatalogList(request: Request, kind: CatalogKind) {
  ...
  try {
    if (kind === "category") {
      ...
      return successList(items.map((item) => ({ ...item, villaCount: item._count.villas })), "database");
    }
    if (kind === "amenity") {
      const items = await prisma.amenity.findMany({
        where: {
          ...(active !== null ? { isActive: active === "true" } : {}),
          ...(search
            ? { OR: [{ name: { contains: search, mode: "insensitive" } }, { group: { contains: search, mode: "insensitive" } }] }
            : {}),
        },
        include: { _count: { select: { villas: true } } },
        orderBy: [{ sortOrder: "asc" }, { name: "asc" }],
    ...
    if (!isPrismaDatabaseUnavailableError(error)) return serverError(kind, error);
...
export async function handleCatalogCreate(request: Request, kind: CatalogKind) {
  ...
  const schema = kind === "category" ? categorySchema : kind === "amenity" ? amenitySchema : promotionSchema;
  ...
  try {
    ...
    const item = await prisma.promotion.create({ data: promotionDatabaseData(data) });
    return created(item, "database");
  } catch (error) {
    ...
    if (kind === "category") {
      const data = parsed.data as z.infer<typeof categorySchema>;
      if (listCatalogCategories().some((item) => item.slug === (data.slug ?? slugifyMasterData(data.name)))) return conflict();
      return created(createCatalogCategory({
        ...data,
    ...
    if (kind === "amenity") {
      const data = parsed.data as z.infer<typeof amenitySchema>;
      if (listCatalogAmenities().some((item) => item.slug === (data.slug ?? slugifyMasterData(data.name)))) return conflict();
      return created(createCatalogAmenity({
        ...data,
        slug: data.slug ?? slugifyMasterData(data.name),
        description: data.description ?? null,
    ...
    const data = parsed.data as z.infer<typeof promotionSchema>;
    if (listCatalogPromotions().some((item) => item.code === data.code)) return conflict();
    return created(createCatalogPromotion({
      ...data,
      description: data.description ?? null,
...
export async function handleCatalogPatch(request: Request, kind: CatalogKind, id: string) {
  if (!isAdmin(request)) return forbidden();
  const body = await readJson(request);
  if (!body.ok) return body.response;
  const normalized = normalizePayload(kind, body.value);
  const schema = kind === "category" ? categorySchema.partial() : kind === "amenity" ? amenitySchema.partial() : promotionPatchSchema();
  const parsed = schema.safeParse(normalized);
  if (!parsed.success) return validationError(parsed.error);
  if (!Object.keys(parsed.data).length) return NextResponse.json({ message: "Tidak ada perubahan." }, { status: 400 });
  ...
  try {
    if (kind === "category") {
      ...
      const item = await prisma.category.update({
        where: { id },
        data: { ...data, ...(data.name && !data.slug ? { slug: slugifyMasterData(data.name) } : {}) },
      });
    ...
    if (isUniqueConflict(error)) return conflict();
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") return notFound();
    if (!isPrismaDatabaseUnavailableError(error)) return serverError(kind, error);
    const item = kind === "category"
      ? updateCatalogCategory(id, parsed.data)
      : kind === "amenity"
        ? updateCatalogAmenity(id, parsed.data)
        : updateCatalogPromotion(id, parsed.data as Partial<ReturnType<typeof listCatalogPromotions>[number]>);
    return item ? updated(item, "memory-fallback") : notFound();
  }
}

export async function handleCatalogDelete(request: Request, kind: CatalogKind, id: string) {
  if (!isAdmin(request)) return forbidden();

  try {
    if (kind === "category") {
      const item = await prisma.category.findUnique({ where: { id }, include: { _count: { select: { villas: true } } } });
      ...
      const item = await prisma.amenity.findUnique({ where: { id }, include: { _count: { select: { villas: true } } } });
      if (!item) return notFound();
      if (item._count.villas) {
    ...
    return NextResponse.json({ message: "Data berhasil dihapus.", mode: "deleted" });
  } catch (error) {
    if (!isPrismaDatabaseUnavailableError(error)) return serverError(kind, error);
    const before = kind === "category" ? listCatalogCategories() : kind === "amenity" ? listCatalogAmenities() : listCatalogPromotions();
    const current = before.find((item) => item.id === id || ("slug" in item && item.slug === id) || ("code" in item && item.code.toLowerCase() === id.toLowerCase()));
    if (!current) return notFound();
    const item = kind === "category" ? deleteCatalogCategory(id) : kind === "amenity" ? deleteCatalogAmenity(id) : deleteCatalogPromotion(id);
    if (!item) return notFound();
    const active = "isActive" in item ? item.isActive : false;
    return NextResponse.json({
      message: active ? "Data berhasil dihapus." : "Data dinonaktifkan karena sudah digunakan.",
...
function normalizePayload(kind: CatalogKind, value: unknown) {
  ...
  return {
    ...input,
    discountType: rawType === "PERCENT" ? "PERCENTAGE" : rawType === "FIXED" ? "FIXED_AMOUNT" : rawType,
...
function promotionPatchSchema() {
  return z.object({
    ...
    code: z.string().trim().min(4).max(40).transform((value) => value.toUpperCase()).optional(),
    description: z.string().trim().max(2000).nullable().optional(),
...
function promotionDatabaseData(data: z.infer<typeof promotionSchema>): Prisma.PromotionCreateInput {
  return {
    ...data,
...
function promotionDatabasePatch(data: Partial<z.infer<typeof promotionSchema>>): Prisma.PromotionUpdateInput {
  return {
    ...data,
    ...(data.startsAt ? { startsAt: new Date(`${data.startsAt}T00:00:00.000Z`) } : {}),
    ...(data.endsAt ? { endsAt: new Date(`${data.endsAt}T23:59:59.999Z`) } : {}),
  };
}

function fallbackList(kind: CatalogKind) {
  return kind === "category" ? listCatalogCategories() : kind === "amenity" ? listCatalogAmenities() : listCatalogPromotions();
}
...
async function readJson(request: Request) {
  try {
    ...
    return { ok: false as const, response: NextResponse.json({ message: "Body request harus JSON valid." }, { status: 400 }) };
  }
}
...
function validationError(error: z.ZodError) {
  return NextResponse.json({ message: "Data master belum valid.", errors: error.flatten().fieldErrors }, { status: 400 });
}
...
function conflict() {
  return NextResponse.json({ message: "Nama, slug, atau kode sudah digunakan." }, { status: 409 });
}
function deactivated() {
  return NextResponse.json({ message: "Data dinonaktifkan karena sudah digunakan.", mode: "deactivated" });
}
...
function serverError(kind: CatalogKind, error: unknown) {
  ...
  return NextResponse.json({ message: "Data master belum dapat diproses." }, { status: 500 });
}
