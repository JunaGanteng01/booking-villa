...
export async function POST(req: Request) {
  try {
    ...
    const { villaId, title, rating, comment } = validationResult.data;
    ...
    if (!pastBooking) {
      return NextResponse.json(
        {
          message:
            "Anda harus pernah memesan villa ini sebelum memberikan ulasan.",
        },
        { status: 403 },
      );
    }
    ...
    return NextResponse.json(
      { review, message: "Ulasan berhasil dikirim." },
      { status: 201 },
    );
  } catch (error) {
