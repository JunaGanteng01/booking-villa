...
type AdminNotificationFilter =
  "all" | "unread" | "action" | AdminNotificationCategory;
...
export default function AdminNotificationHistoryPage() {
  ...
  useEffect(() => {
    ...
    setNotifications(getStoredAdminNotifications(profile.email));
    setSelected([]);
  }, [profile.email]);
  ...
  const accessibleNotifications = useMemo(
    () =>
      notifications.filter((notification) =>
        canViewNotification(profile.role, notification.category),
      ),
    [notifications, profile.role],
  );
  const unreadCount = accessibleNotifications.filter(
    (notification) => notification.unread,
  ).length;
  const actionCount = accessibleNotifications.filter(
    (notification) => notification.requiresAction,
  ).length;
  const urgentCount = accessibleNotifications.filter(
  ...
  const visibleNotifications = useMemo(() => {
    ...
    return notifications.filter((notification) => {
      if (!canViewNotification(profile.role, notification.category))
        return false;
      const matchesFilter =
        filter === "all" ||
  ...
  }, [filter, notifications, profile.role, query]);
  ...
  const toggleAllVisible = () => {
    const visibleIds = visibleNotifications.map(
  ...
  const markRead = (ids: string[]) => {
    ...
    setNotifications((items) => {
      ...
      persistAdminNotifications(profile.email, next);
      return next;
    });
    setSelected([]);
    notify({
      ...
      <div className="flex min-h-screen">
              ...
              return (
                ...
                >
                  ...
                  {item.active && unreadCount > 0 ? (
                    <span className="rounded-full bg-white/16 px-2 py-0.5 text-[0.62rem]">
                      {unreadCount}
                    </span>
                  ) : null}
                </Link>
        ...
        <div className="min-w-0 flex-1 lg:pl-64">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 hidden text-xs text-emerald-950/42 dark:text-white/40 sm:block">
                    Pantau aktivitas operasional Villaku
                  </p>
                </div>
              </div>
                        ...
                        <Link
                          ...
                          className={cn(
                            ...
                            item.active
                              ...
                              : "text-emerald-950/54 dark:text-white/52",
                          )}
                        >
          ...
          <div className="mx-auto max-w-[1440px] px-4 pb-14 pt-7 sm:px-6 lg:px-8 lg:pt-10">
            ...
            >
              <div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-emerald-950/48 dark:text-white/46">
                  Prioritaskan pembayaran, booking, dan kebutuhan tamu yang
                  memerlukan respons tim.
                </p>
              </div>
            ...
            <section
              className="mt-7 overflow-hidden rounded-[1.7rem] border border-emerald-950/8 bg-white/68 shadow-[0_20px_70px_rgba(4,34,28,0.055)] backdrop-blur-xl dark:border-white/8 dark:bg-white/[0.045]"
              ...
              <div className="border-b border-emerald-950/8 p-4 dark:border-white/8 sm:p-5">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div
                    ...
                    role="tablist"
                    aria-label="Filter notifikasi admin"
                  >
                    {filters.map((item) => (
                      <button
                        type="button"
                        ...
                        className={cn(
                          "min-h-10 shrink-0 rounded-full px-4 text-xs font-bold transition-all",
                          filter === item.id
                            ? "bg-emerald-700 text-white"
                            : "bg-emerald-950/5 text-emerald-950/52 hover:bg-emerald-950/8 dark:bg-white/6 dark:text-white/50 dark:hover:bg-white/9",
                        )}
                      >
                        {item.label}
                      </button>
                  ...
                  {selected.length ? (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="inline-flex min-h-8 items-center gap-1.5 rounded-full bg-emerald-100 px-3 text-xs font-semibold text-emerald-700 dark:bg-emerald-300/10 dark:text-emerald-200"
                      >
                      ...
                      <button
                        ...
                        className="min-h-8 rounded-full bg-emerald-950/5 px-3 text-xs font-semibold text-emerald-950/54 dark:bg-white/6 dark:text-white/52"
                      >
                        Arsipkan
                      </button>
                    </motion.div>
                    ...
                    <motion.p
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="text-xs text-emerald-950/36 dark:text-white/34"
                    >
                      {visibleNotifications.length} aktivitas ditampilkan
                    </motion.p>
                  )}
                </AnimatePresence>
              </div>
                ...
                <AnimatePresence mode="wait">
                  {visibleNotifications.length ? (
                    ...
                    <motion.div
                      ...
                      className="grid min-h-96 place-items-center px-5 text-center"
                    >
                      <div>
                        ...
                        <p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-emerald-950/46 dark:text-white/44">
                          Ubah filter atau kata kunci untuk melihat aktivitas
                          operasional lainnya.
                        </p>
                        <button
                          type="button"
...
function MetricCard({
  ...
  const tones = {
    emerald:
      "border-emerald-700/12 bg-emerald-100/62 text-emerald-800 dark:bg-emerald-300/8 dark:text-emerald-200",
    amber:
      "border-amber-600/12 bg-amber-100/62 text-amber-800 dark:bg-amber-300/8 dark:text-amber-200",
    rose: "border-rose-600/12 bg-rose-100/62 text-rose-800 dark:bg-rose-300/8 dark:text-rose-200",
  };
  return (
    <div
      className={cn(
        "min-w-0 rounded-2xl border p-3 sm:min-w-32 sm:p-4",
        tones[tone],
      )}
    >
      <p className="text-2xl font-semibold sm:text-3xl">{value}</p>
      <p className="mt-1 truncate text-[0.62rem] font-bold uppercase tracking-[0.1em] opacity-60 sm:text-[0.65rem]">
        {label}
      </p>
    </div>
  );
...
function AdminNotificationRow({
  ...
  return (
    <motion.article
      ...
      className={cn(
        "group grid gap-3 border-b border-emerald-950/7 p-4 transition-colors last:border-b-0 hover:bg-emerald-950/[0.025] dark:border-white/7 dark:hover:bg-white/[0.025] sm:grid-cols-[auto_auto_minmax(0,1fr)_auto] sm:items-start sm:gap-4 sm:p-5",
        selected && "bg-emerald-50 dark:bg-emerald-300/[0.045]",
        notification.unread && "bg-white/50 dark:bg-white/[0.018]",
      )}
    >
      <input
        type="checkbox"
        checked={selected}
        onChange={onSelect}
        className="mt-1 size-4 rounded border-emerald-950/20 accent-emerald-700"
        aria-label={`Pilih ${notification.title}`}
      />
      <span
        className={cn(
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {notification.priority === "urgent" ? (
            <span className="inline-flex items-center gap-1 rounded-full bg-rose-100 px-2 py-0.5 text-[0.58rem] font-bold uppercase tracking-[0.1em] text-rose-700 dark:bg-rose-300/10 dark:text-rose-200">
              <AlertTriangle className="size-3" /> Urgent
            </span>
          ) : null}
          {notification.requiresAction ? (
            <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[0.58rem] font-bold uppercase tracking-[0.1em] text-amber-700 dark:bg-amber-300/10 dark:text-amber-200">
              Perlu tindakan
            </span>
          ) : null}
          {notification.unread ? (
        ...
        <h2
          className={cn(
            "mt-1.5 text-sm leading-5 text-emerald-950 dark:text-white sm:text-base",
            notification.unread ? "font-bold" : "font-semibold",
          )}
        >
          {notification.title}
        </h2>
        <p className="mt-1.5 max-w-3xl text-sm leading-6 text-emerald-950/48 dark:text-white/44">
      ...
      <div className="flex items-center gap-2 sm:justify-end">
        {notification.unread ? (
          <button
            type="button"
            onClick={onRead}
            className="grid size-9 place-items-center rounded-full bg-emerald-950/5 text-emerald-700 transition hover:bg-emerald-100 dark:bg-white/6 dark:text-emerald-200"
            aria-label={`Tandai ${notification.title} telah dibaca`}
          >
        ...
        <Link
          href={href}
          onClick={onOpen}
          className="inline-flex min-h-9 items-center gap-1 rounded-full border border-emerald-950/10 bg-white/70 px-3 text-xs font-bold text-emerald-800 transition-all hover:border-emerald-700/20 hover:bg-white dark:border-white/10 dark:bg-white/6 dark:text-white dark:hover:bg-white/10"
        >
